...
std::shared_ptr<SceneNode> GLTFLoader::Parse(const std::filesystem::path& path) {
...
void GLTFLoader::LoadBuffers(const nlohmann::json& gltf_json) {
...
void GLTFLoader::LoadBufferViews(const nlohmann::json& gltf_json) {
...
void GLTFLoader::LoadAccessors(const nlohmann::json& gltf_json) {
...
void GLTFLoader::LoadImages(const nlohmann::json& gltf_json) {
...
void GLTFLoader::LoadTextures(const nlohmann::json& gltf_json) {
...
void GLTFLoader::LoadMaterials(const nlohmann::json& gltf_json) {
...
void GLTFLoader::LoadMeshes(const nlohmann::json& gltf_json) {
...
void GLTFLoader::LoadVec3Accessor(const Accessor& accessor, Geometry::Accessor<Vector3> geometry_accessor) {
...
void GLTFLoader::LoadIndexBufferAccessor(const Accessor& accessor, std::span<u32> geometry_index_buffer) {
  // TODO: lots of validation... all of this is unsafe.
  const BufferView& buffer_view = m_buffer_views[accessor.buffer_view];
  const Buffer& buffer = m_buffers[buffer_view.buffer];
...
std::shared_ptr<SceneNode> GLTFLoader::LoadNodeHierarchy(const nlohmann::json& gltf_json, size_t node_index) {
